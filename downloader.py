...
# ==========================================
# GÜVENLİ YAPILANDIRMA VE TEMİZLEME
# ==========================================
def clean_env(key):
    val = os.environ.get(key, "")
    # Secret içindeki her türlü yabancı karakteri temizle
...
def send_telegram(message):
    """URL'yi en saf haliyle kurar ve mesajı parçalı gönderir."""
    # URL'yi parantezsiz ve tertemiz kuruyoruz
    ...
    for part in parts:
        ...
        try:
            # URL'yi string tipine zorlayarak gönderiyoruz
            resp = requests.post(str(api_url), json=payload, timeout=30)
            if resp.status_code != 200:
                # Markdown hatası ihtimaline karşı düz metin dene
                requests.post(str(api_url), json={"chat_id": TELEGRAM_CHAT_ID, "text": part}, timeout=30)
...
def get_ai_analysis(current_pdf_text, previous_summary, report_type):
    try:
        ...
        4. TABLOLARI mutlaka ``` (triple backticks) içine al (monospace).
...
def process_automation():
    ...
    with sync_playwright() as p:
        ...
        # Browser context'i daha güvenli tanımlıyoruz
        context = browser.new_context(user_agent="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)")
        page = context.new_page()
        
        # URL'yi bir değişkene atayarak karışıklığı önledik
        target_web_url = "[https://www.garantibbvayatirim.com.tr/arastirma-raporlari](https://www.garantibbvayatirim.com.tr/arastirma-raporlari)"
        
        try:
            print(f"--- Siteye Gidiliyor: {target_web_url} ---")
            page.goto(target_web_url, wait_until="domcontentloaded", timeout=60000)
